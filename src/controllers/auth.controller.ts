...
import nodemailer from 'nodemailer'; // ✉️ Envio de e-mails

// 🔧 Configuração da Evolution API
const EVOLUTION_URL =
  'https://evolution20.sgesoftware.com.br/message/sendText/Suporte_SGE';
  ...
  try {
    ...
    if (!usuario.senha_md5) {
      return res.status(200).json({
        ...
        mensagem:
          'Usuário sem senha cadastrada. Crie uma senha para continuar.',
        usuario: {
          id: usuario.id,
...
export async function forgotPassword(req: Request, res: Response) {
  try {
    ...
    return res.json({
      message:
        'Token de recuperação gerado e enviado via WhatsApp e e-mail.',
      expira_em: expiresAt,
    });
  } catch (error) {
