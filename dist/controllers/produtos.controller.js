...
exports.searchProduto = searchProduto;
...
const isBarcode_1 = require("../utils/isBarcode");
/**
 * ?? Busca e pesquisa produtos (por nome, c��digo ERP ou c��digo de barras)
 */
async function searchProduto(req, res) {
    try {
        ...
        if (!cod_loja) {
            return res.status(400).json({ error: 'Parametro cod_loja �� obrigat��rio' });
        }
        // Caso sem busca: retorna todos
        if (!q_raw) {
            const total = await prismaClient_1.prisma.produto.count({ where: { cod_loja } });
        ...
        // Busca por c��digo de barras (produto ou EAN)
        if ((0, isBarcode_1.isBarcode)(q_raw)) {
            const prodPorCodigo = await prismaClient_1.prisma.produto.findMany({
            ...
            if (codProdutos.length > 0) {
                const prods = await prismaClient_1.prisma.produto.findMany({
                    where: { cod_loja, codigo: { in: codProdutos } },
                });
                return res.json({ total: prods.length, data: prods });
            }
        }
        // Busca por nome ou c��digo ERP
        const maybeNumber = Number(q_raw);
        const where = { cod_loja, OR: [] };
        if (!isNaN(maybeNumber))
            where.OR.push({ codigo: maybeNumber });
        where.OR.push({ nome: { contains: q_raw, mode: 'insensitive' } });
        const total = await prismaClient_1.prisma.produto.count({ where });
        const produtos = await prismaClient_1.prisma.produto.findMany({
            where,
            orderBy: { nome: 'asc' },
            ...(limit ? { take: limit, skip: offset } : {}),
...
async function listarEans(req, res) {
    try {
        ...
        const eans = await prismaClient_1.prisma.ean.findMany({ where: { cod_loja } });
...
/**
 * ? Cadastra um ou v��rios produtos
 */
    ...
    try {
        ...
        for (const p of produtos) {
            if (!p.cod_loja || !p.codigo || !p.nome || !p.unidade_medida) {
                return res.status(400).json({
                    error: 'Campos obrigat��rios: cod_loja, codigo, nome e unidade_medida.',
                });
            }
        }
        // Cria os produtos (sem verificar duplicados)
        const created = await prismaClient_1.prisma.produto.createMany({
            data: produtos.map(p => ({
                codigo: p.codigo, // c��digo ERP
                cod_loja: p.cod_loja,
                nome: p.nome,
        ...
        for (const p of produtos) {
            if (Array.isArray(p.eans) && p.eans.length > 0) {
                await prismaClient_1.prisma.ean.createMany({
                    data: p.eans.map((ean) => ({
                        cod_loja: p.cod_loja,
                        cod_produto: p.codigo, // ?? usa o c��digo do produto (ERP)
                        codigo_barras: ean,
                    })),
                });
            }
...
async function atualizarProduto(req, res) {
    try {
        ...
        const produto = await prismaClient_1.prisma.produto.findUnique({ where: { id } });
        if (!produto)
            return res.status(404).json({ error: 'Produto n?o encontrado' });
        const atualizado = await prismaClient_1.prisma.produto.update({
            where: { id },
            data: {
...
async function excluirProduto(req, res) {
    try {
        ...
        if (!produto)
            return res.status(404).json({ error: 'Produto n?o encontrado' });
        // Exclui EANs com base no c��digo ERP e loja
        await prismaClient_1.prisma.ean.deleteMany({
            where: { cod_produto: produto.codigo, cod_loja: produto.cod_loja },
        });
        await prismaClient_1.prisma.produto.delete({ where: { id } });
